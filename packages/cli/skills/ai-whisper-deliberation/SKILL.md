...
Once kicked off, the workflow runs: a research/synthesis phase where one agent gathers evidence (from the codebase and/or the web) and drafts the findings; an adversarial review phase where a second agent stress-tests the findings for gaps, contradictions, and unsupported claims; and a findings-consolidation phase producing the final document. Watch all of this on `whisper collab dashboard`; do not babysit it from chat.
