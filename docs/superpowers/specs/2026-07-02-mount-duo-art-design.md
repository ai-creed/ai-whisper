...
3. **Codex mitigations:** after printing art, hold a dramatic pause (~2s), then print `stdout.rows` newlines to push the art off the top into scrollback before the spawn ("scrollback-push"). The pause makes the summoning visible; the push preserves the art in history.
