...
**`deliberation`** is an open-ended dialectic. An Explorer and a Challenger propose and adversarially stress-test across Objectives → Approaches → Tradeoffs → Synthesis; a findings doc is committed in the final Synthesis phase. The output is a structured handover into brainstorming or SDD; it never auto-chains. Use it when you have a fuzzy idea, cannot yet describe "done," and want the space mapped before you commit to a spec.
...
Avoid: a seed that is really a spec (you already know the solution), a seed so vague it could mean anything (the Explorer and Challenger will struggle to converge), or a topic with no project grounding at all — deliberation still runs but degrades to web-grounded research, with weaker verification and more open questions.
